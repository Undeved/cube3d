...
static void init_hovered_versions(t_parsed_data *pd)
{
    ...
    if (!pd->menu.b_start_hv.txtr || !pd->menu.b_chars_hv.txtr || !pd->menu.b_exit_hv.txtr)
        ui_error();

    pd->menu.b_start_hv.img = mlx_texture_to_image(pd->mlx, pd->menu.b_start_hv.txtr);
    if (!pd->menu.b_start_hv.img || mlx_image_to_window(pd->mlx, pd->menu.b_start_hv.img,
            START_BUTTON_X, START_BUTTON_Y))
        mind_free_all(EXIT_FAILURE);

    pd->menu.b_chars_hv.img = mlx_texture_to_image(pd->mlx, pd->menu.b_chars_hv.txtr);
    if (!pd->menu.b_chars_hv.img || mlx_image_to_window(pd->mlx, pd->menu.b_chars_hv.img,
            CHARS_BUTTON_X, CHARS_BUTTON_Y))
        mind_free_all(EXIT_FAILURE);

    pd->menu.b_exit_hv.img = mlx_texture_to_image(pd->mlx, pd->menu.b_exit_hv.txtr);
    if (!pd->menu.b_exit_hv.img || mlx_image_to_window(pd->mlx, pd->menu.b_exit_hv.img, 
...
{
    pd->menu.b_start.hoverd = true; // by default start is selected.
...
static void init_buttons(t_parsed_data *pd)
{
    ...
    if (!pd->menu.b_start.txtr || !pd->menu.b_chars.txtr || !pd->menu.b_exit.txtr)
        ui_error();

    pd->menu.b_start.img = mlx_texture_to_image(pd->mlx, pd->menu.b_start.txtr);
    if (!pd->menu.b_start.img || mlx_image_to_window(pd->mlx, pd->menu.b_start.img,
            START_BUTTON_X, START_BUTTON_Y - BTN_POP))
        mind_free_all(EXIT_FAILURE);

    pd->menu.b_chars.img = mlx_texture_to_image(pd->mlx, pd->menu.b_chars.txtr);
    if (!pd->menu.b_chars.img || mlx_image_to_window(pd->mlx, pd->menu.b_chars.img,
            CHARS_BUTTON_X, CHARS_BUTTON_Y - BTN_POP))
        mind_free_all(EXIT_FAILURE);

    pd->menu.b_exit.img = mlx_texture_to_image(pd->mlx, pd->menu.b_exit.txtr);
    if (!pd->menu.b_exit.img || mlx_image_to_window(pd->mlx, pd->menu.b_exit.img,
...
void    init_main_menu(t_parsed_data *pd)
{
    ...
    if (!pd->menu.bg.img || mlx_image_to_window(pd->mlx, pd->menu.bg.img, 0, 0) == -1)
        mind_free_all(EXIT_FAILURE);
    pd->menu.title.img = mlx_texture_to_image(pd->mlx, pd->menu.title.txtr);
    if (!pd->menu.title.img || mlx_image_to_window(pd->mlx, pd->menu.title.img, TITLE_X, TITLE_Y) == -1)
        mind_free_all(EXIT_FAILURE);
    init_buttons(pd);
