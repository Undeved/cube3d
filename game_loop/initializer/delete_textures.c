...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/10 03:10:54 by oimzilen          #+#    #+#             */
/*   Updated: 2025/11/10 03:12:19 by oimzilen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	free_shared_attack(t_parsed_data *pd)
{
	...
	while (i < 3)
	{
		...
		if (pd->sh.skin_walker_attack[i].img)
			mlx_delete_image_set_null(pd->mlx,
				&pd->sh.skin_walker_attack[i].img);
		...
		if (pd->sh.memory_leak_attack[i].img)
			mlx_delete_image_set_null(pd->mlx,
				&pd->sh.memory_leak_attack[i].img);
...
static void	free_sh_imgs(t_parsed_data *pd)
{
	...
	while (i < 2)
	{
		if (pd->sh.skin_walker_death[i].img)
			mlx_delete_image_set_null(pd->mlx,
				&pd->sh.skin_walker_death[i].img);
		if (pd->sh.memory_leak_death[i].img)
			mlx_delete_image_set_null(pd->mlx,
				&pd->sh.memory_leak_death[i].img);
