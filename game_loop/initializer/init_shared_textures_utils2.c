...
static void default_textures_to_images(t_parsed_data *pd)
{
    ...
    {
        pd->sh.skin_walker_default.img = mlx_texture_to_image(pd->mlx, pd->sh.skin_walker_default.txtr);
        if (!pd->sh.skin_walker_default.img)
            ui_error();
    }
    if (pd->sh.memory_leak_default.txtr && !pd->sh.memory_leak_default.img)
    {
        pd->sh.memory_leak_default.img = mlx_texture_to_image(pd->mlx,  pd->sh.memory_leak_default.txtr);
        if (!pd->sh.memory_leak_default.img)
            ui_error();
    }
    if (pd->sh.segv_default.txtr && !pd->sh.segv_default.img)
    {
        pd->sh.segv_default.img = mlx_texture_to_image(pd->mlx,  pd->sh.segv_default.txtr);
