...
static void	init_game_ui(t_parsed_data *pd)
{
	...
	if (!pd->game_ui.game_over.txtr)
		ui_error();
	pd->game_ui.game_over.img = mlx_texture_to_image(pd->mlx,
			pd->game_ui.game_over.txtr);
	if (!pd->game_ui.game_over.img
		|| mlx_image_to_window(pd->mlx, pd->game_ui.game_over.img, 0, 0) == -1)
...
static void	init_game_won_ui(t_parsed_data *pd)
{
	...
	if (!pd->game_ui.game_won.txtr)
		ui_error();
	pd->game_ui.game_won.img = mlx_texture_to_image(pd->mlx,
			pd->game_ui.game_won.txtr);
