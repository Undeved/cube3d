...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   setup_character_gun_shoot.c                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: oimzilen <oimzilen@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/10 01:40:34 by oimzilen          #+#    #+#             */
/*   Updated: 2025/11/10 01:48:19 by oimzilen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	load_gun_shoot_frame(t_parsed_data *pd, int i, char *path)
{
	...
	if (!pd->player.gun.shoot.frames[i].txtr)
		ui_error();
	pd->player.gun.shoot.frames[i].img = mlx_texture_to_image(pd->mlx,
			pd->player.gun.shoot.frames[i].txtr);
	if (!pd->player.gun.shoot.frames[i].img || mlx_image_to_window(pd->mlx,
			pd->player.gun.shoot.frames[i].img, GUN_AIM_X, GUN_AIM_Y) != 0)
		ui_error();
...
void	load_gun_shoot_anim(t_parsed_data *pd, t_character character)
{
	...
	pd->player.gun.shoot.frames = allocate_gc(malloc(sizeof(t_raw_img)
				* GUN_SHOOT_FRMS));
