...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   setup_character_gun_reload.c                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: oimzilen <oimzilen@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/10 01:25:07 by oimzilen          #+#    #+#             */
/*   Updated: 2025/11/10 01:34:11 by oimzilen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	load_gun_reload_frame(t_parsed_data *pd, int i, char *path)
{
	...
	if (!pd->player.gun.reload.frames[i].txtr)
		ui_error();
	pd->player.gun.reload.frames[i].img = mlx_texture_to_image(pd->mlx,
			pd->player.gun.reload.frames[i].txtr);
	if (!pd->player.gun.reload.frames[i].img
		|| mlx_image_to_window(pd->mlx, pd->player.gun.reload.frames[i].img,
			GUN_X, GUN_Y) != 0)
		ui_error();
...
void	load_gun_reload_anim(t_parsed_data *pd, t_character character)
{
	...
	pd->player.gun.reload.frames = allocate_gc(malloc(sizeof(t_raw_img)
				* GUN_RELOAD_FRMS));
