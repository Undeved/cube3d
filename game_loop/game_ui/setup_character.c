...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/10 01:48:38 by oimzilen          #+#    #+#             */
...
static void	load_gun_idle_png_failure(t_parsed_data *pd, char *path)
{
	...
	if (!pd->game_ui.gun.txtr)
		ui_error();
	pd->game_ui.gun.img = mlx_texture_to_image(pd->mlx,
			pd->game_ui.gun.txtr);
	if (!pd->game_ui.gun.img
		|| mlx_image_to_window(pd->mlx, pd->game_ui.gun.img, GUN_X, GUN_Y) != 0)
		ui_error();
}
...
static void	load_gun_aim_png_failure(t_parsed_data *pd, char *path)
{
	...
	if (!pd->game_ui.gun_aim.txtr)
		ui_error();
	pd->game_ui.gun_aim.img = mlx_texture_to_image(pd->mlx,
			pd->game_ui.gun_aim.txtr);
	if (!pd->game_ui.gun_aim.img
		|| mlx_image_to_window(pd->mlx, pd->game_ui.gun_aim.img,
			GUN_AIM_X, GUN_AIM_Y) != 0)
		ui_error();
...
static void	load_gun_no_ammo_png_failure(t_parsed_data *pd, char *path)
{
	...
	if (!pd->game_ui.no_ammo.txtr)
		ui_error();
	pd->game_ui.no_ammo.img = mlx_texture_to_image(pd->mlx,
			pd->game_ui.no_ammo.txtr);
	if (!pd->game_ui.no_ammo.img
		|| mlx_image_to_window(pd->mlx, pd->game_ui.no_ammo.img,
			GUN_X, GUN_Y) != 0)
		ui_error();
