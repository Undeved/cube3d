...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   init_characters_ui.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: oimzilen <oimzilen@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/09 22:49:22 by oimzilen          #+#    #+#             */
/*   Updated: 2025/11/09 22:55:35 by oimzilen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_hovered_char(t_parsed_data *pd)
{
	...
	if (!pd->chars_menu.b_jesse_hv.txtr || !pd->chars_menu.b_chorus_hv.txtr
		|| !pd->chars_menu.b_oussmac_hv.txtr)
		ui_error();
	pd->chars_menu.b_jesse_hv.img = mlx_texture_to_image(pd->mlx,
			pd->chars_menu.b_jesse_hv.txtr);
	if (!pd->chars_menu.b_jesse_hv.img
		|| mlx_image_to_window(pd->mlx, pd->chars_menu.b_jesse_hv.img,
			JESSE_X, JESSE_Y))
		mind_free_all(EXIT_FAILURE);
	pd->chars_menu.b_chorus_hv.img = mlx_texture_to_image(pd->mlx,
			pd->chars_menu.b_chorus_hv.txtr);
	if (!pd->chars_menu.b_chorus_hv.img
		|| mlx_image_to_window(pd->mlx, pd->chars_menu.b_chorus_hv.img,
			CHORUS_X, CHORUS_Y))
		mind_free_all(EXIT_FAILURE);
	pd->chars_menu.b_oussmac_hv.img = mlx_texture_to_image(pd->mlx,
			pd->chars_menu.b_oussmac_hv.txtr);
	if (!pd->chars_menu.b_oussmac_hv.img
		|| mlx_image_to_window(pd->mlx, pd->chars_menu.b_oussmac_hv.img,
			OUSSMAC_X, OUSSMAC_Y))
		mind_free_all(EXIT_FAILURE);
}
...
static void	init_char_buttons(t_parsed_data *pd)
{
	...
	if (!pd->chars_menu.b_jesse.txtr || !pd->chars_menu.b_chorus.txtr
		|| !pd->chars_menu.b_oussmac.txtr)
		ui_error();
	pd->chars_menu.b_jesse.img = mlx_texture_to_image(pd->mlx,
			pd->chars_menu.b_jesse.txtr);
	if (!pd->chars_menu.b_jesse.img
		|| mlx_image_to_window(pd->mlx, pd->chars_menu.b_jesse.img,
			JESSE_X, JESSE_Y - BTN_POP))
		mind_free_all(EXIT_FAILURE);
	pd->chars_menu.b_chorus.img = mlx_texture_to_image(pd->mlx,
			pd->chars_menu.b_chorus.txtr);
	if (!pd->chars_menu.b_chorus.img
		|| mlx_image_to_window(pd->mlx, pd->chars_menu.b_chorus.img,
			CHORUS_X, CHORUS_Y - BTN_POP))
		mind_free_all(EXIT_FAILURE);
	pd->chars_menu.b_oussmac.img = mlx_texture_to_image(pd->mlx,
			pd->chars_menu.b_oussmac.txtr);
	if (!pd->chars_menu.b_oussmac.img
		|| mlx_image_to_window(pd->mlx, pd->chars_menu.b_oussmac.img,
...
static void	init_selected_button(t_parsed_data *pd)
{
	...
	if (!pd->chars_menu.selected.txtr)
		ui_error();
	pd->chars_menu.selected.img = mlx_texture_to_image(pd->mlx,
			pd->chars_menu.selected.txtr);
	if (!pd->chars_menu.selected.img
		|| mlx_image_to_window(pd->mlx, pd->chars_menu.selected.img,
			SELECT_JESSE_X, SELECT_Y) == -1)
		mind_free_all(EXIT_FAILURE);
...
void	init_characters_menu(t_parsed_data *pd)
{
	...
	if (!pd->chars_menu.bg.txtr)
		ui_error();
	pd->chars_menu.bg.img = mlx_texture_to_image(pd->mlx,
			pd->chars_menu.bg.txtr);
	if (!pd->chars_menu.bg.img
		|| mlx_image_to_window(pd->mlx, pd->chars_menu.bg.img, 0, 0) == -1)
		mind_free_all(EXIT_FAILURE);
